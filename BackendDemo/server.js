...
// Serve static files from root and LogoBank directory
const path = require('path');
const rootPath = path.join(__dirname, '..');
app.use(express.static(rootPath));
app.use('/LogoBank', express.static(path.join(rootPath, 'LogoBank')));
...
app.get('/api/user/balance', (req, res) => {
    ...
    if (!user) return res.status(401).json({ balance: 0 }); // Default to 0 if not logged in
    res.json({ balance: user.balance });
});

app.get('/api/user/transactions', (req, res) => {
    const userId = req.headers['x-user-id'];
    const userTransactions = transactions.filter(t => t.userId === userId).slice().reverse();
...
app.post('/api/payment/free', (req, res) => {
    ...
    if (packageId === 'p0') {
        const hasBoughtFreeP0 = transactions.some(t => t.userId === userId && t.packageId === 'p0' && t.status === 'success');
        if (hasBoughtFreeP0) {
    ...
    transactions.push({ orderId: 'FREE' + Date.now(), userId, packageId, status: 'success', provider: 'free' });
    res.json({ success: true, message: 'Nhận gói miễn phí thành công!', balance: user.balance });
});
...
app.post('/api/payment/vnpay', (req, res) => {
    ...
    if (finalPrice === 0) {
        user.balance += pkg.amount;
        transactions.push({ orderId: 'FREE_VNPAY' + Date.now(), userId, packageId, status: 'success', provider: 'vnpay' });
    ...
    transactions.push({ orderId, userId, packageId, status: 'pending', provider: 'vnpay' });
        ...
        if (responseCode === '00') {
            ...
            if (trans) {
                if (trans.status !== 'success') {
                    ...
                    if (pkg) {
                        const user = getUser(trans.userId);
                        user.balance += pkg.amount;
                        trans.status = 'success';
                        console.log(`Balance updated for ${trans.userId}: +${pkg.amount}. New Balance: ${user.balance}`);
                    }
                } else {
            ...
            if (parseInt(vnp_Params['vnp_Amount']) === amount) {
                if (trans.status !== 'success') {
                    if (rspCode === '00') {
                        ...
                        user.balance += pkg.amount;
                        trans.status = 'success';
                        console.log(`IPN: Balance updated: +${pkg.amount}. New Balance: ${user.balance}`);
    ...
    if (resultCode == '0') {
        if (trans && trans.status !== 'success') {
            ...
            if (pkg) {
                ...
                user.balance += pkg.amount;
                trans.status = 'success';
                console.log(`MoMo Return: Balance updated for ${trans.userId}: +${pkg.amount}. New Balance: ${user.balance}`);
            }
        }
        res.redirect(`${frontendUrl}?paymentStatus=success`);
    } else {
...
app.post(['/api/payment/momo', '/api/payment/momo_atm'], async (req, res) => {
    ...
    if (finalPrice === 0) {
        user.balance += pkg.amount;
        transactions.push({ orderId: 'FREE_MOMO' + Date.now(), userId, packageId, status: 'success', provider: isAtm ? 'momo_atm' : 'momo' });
        const frontendUrl = process.env.FRONTEND_URL || 'http://localhost:5173';
        return res.json({ paymentUrl: `${frontendUrl}?paymentStatus=success` });
    }
...
app.post('/api/payment/dev_recharge', (req, res) => {
    ...
    if (packageId === 'dev_success') {
        user.balance += pkg.amount;
        transactions.push({ orderId: 'DEV' + Date.now(), userId, packageId, status: 'success', provider: 'dev' });
        return res.json({ success: true, message: 'Nạp vô hạn tiền thành công!', balance: user.balance });
    } else {
        transactions.push({ orderId: 'DEV' + Date.now(), userId, packageId, status: 'failed', provider: 'dev' });
